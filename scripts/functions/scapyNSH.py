...
call('ip link add vxlan0 type vxlan id 0x1337 remote 192.168.100.104 dev eth0 dstport 6633', shell=True)
call('ip link set vxlan0 up', shell=True)
...
def NSHForward(pkt):
    ...
    print ("pkt --- " + str(pkt[Raw].load))
    next = pkt[NSH]
    next.ServiceIndex = next.ServiceIndex - 1
    d = pkt[Raw]
    ip = get_ip_address(ingress);
    next[Raw].load = load=str(d).strip() + "-" + str(ip)
    print ("next -- " + str(next[Raw].load))
    sendp(next, iface=egress)


def StartSF():
    conf.verb = 0
    print  get_ip_address(ingress)
