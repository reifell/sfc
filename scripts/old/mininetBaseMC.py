...
from old.odlConfGenerationMC import odlConfMC
    ...
    def deploySfc(self):
        for chain in self.callBackConfs['chain']:
            for chainName, conf in chain.iteritems():
                ...
                self.odl.post(self.odl.controller, self.odl.DEFAULT_PORT, self.odl.SERVICE_CLASSIFICATION_FUNTION,
                              conf['scf1'], True)
                self.odl.post(self.odl.controller, self.odl.DEFAULT_PORT, self.odl.SERVICE_CLASSIFICATION_FUNTION,
                          conf['scf2'], True)
        ...
        print scf
        call('ovs-ofctl -OOpenFlow13 add-flow %s priority=99,actions=normal' % (scf),
             shell=True)  # normal traffic from no chain
